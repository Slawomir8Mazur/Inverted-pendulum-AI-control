...
class Record:
    ...
    def PID_single_move(self, Kp, Ki, Kd):
        ...
        if len(self.last_movement) >= 2:
            if np.any(pd.isna(self.last_movement.iloc[-2:]['t'])):
                ...
                output += Ki * dt * (give_error(self.last_movement.iloc[-1]['fi']) + give_error(
                    self.last_movement.iloc[-2]['fi'])) / 2
                if Kd:
                    output += Kd * (give_error(self.last_movement.iloc[-1]['fi']) - give_error(
                        self.last_movement.iloc[-2]['fi'])) / dt
                return output
        else:
            return output
    ...
    def PID_control(self, how_many_iter=5, Kp=100, Ki=0, Kd=0, dt_min=0.01):
        for i in range(how_many_iter):
            self.move([(self.PID_single_move(Kp, Ki, Kd), 5 * dt_min)], dt_min=dt_min)
